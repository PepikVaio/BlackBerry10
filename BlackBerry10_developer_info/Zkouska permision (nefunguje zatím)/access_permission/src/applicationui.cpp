/*
 * Copyright (c) 2011-2015 BlackBerry Limited.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include "applicationui.hpp"

#include <bb/cascades/Application>
#include <bb/cascades/QmlDocument>
#include <bb/cascades/AbstractPane>
#include <bb/cascades/LocaleHandler>

// include custom class
#include "Device/device.hpp"
//**************************
#include "JavaScript/javascript.hpp"
//**********************************
#include "Settings/settings.hpp"
//******************************
#include "SqlDatabase/sqldatabase.hpp"
//******************************
#include "Timer/timer.hpp"
//************************
#include "WebImageView/webimageview.hpp"
//**************************************
#include "XmlReader/xmlreader.hpp"
//********************************



using namespace bb::cascades;

ApplicationUI::ApplicationUI() :
        QObject()
{

    // register a QML type made with C++
    //qmlRegisterType<Device>("custom.Device", 1, 0, "Device");
    //*******************************************************
    // register a QML type made with C++
    //qmlRegisterType<JavaScript>("custom.JavaScript", 1, 0, "JavaScript");
    //*********************************************************************
    //qmlRegisterType<Settings>("custom.Settings", 1, 0, "Settings");
    //*************************************************************
    qmlRegisterType<SqlDatabase>("custom.SqlDatabase", 1, 0, "SqlDatabase");
    //**********************************************************************
    qmlRegisterType<Timer>("custom.Timer", 1, 0, "Timer");
    //****************************************************
    qmlRegisterType<WebImageView>("custom.WebImageView", 1, 0, "WebImageView");
    //*************************************************************************
    //qmlRegisterType<XmlReader>("custom.XmlReader", 1, 0, "XmlReader");
    //****************************************************************

    // prepare the localization
    m_pTranslator = new QTranslator(this);
    m_pLocaleHandler = new LocaleHandler(this);

    bool res = QObject::connect(m_pLocaleHandler, SIGNAL(systemLanguageChanged()), this, SLOT(onSystemLanguageChanged()));
    // This is only available in Debug builds
    Q_ASSERT(res);
    // Since the variable is not used in the app, this is added to avoid a
    // compiler warning
    Q_UNUSED(res);

    // initial load
    onSystemLanguageChanged();

    // Create scene document from main.qml asset, the parent is set
    // to ensure the document gets destroyed properly at shut down.
    QmlDocument *qml = QmlDocument::create("asset:///main.qml").parent(this);

    // add global c++ object to the QML context as a property
    Device *device = new Device();
    qml->setContextProperty("_device", device);
    //*****************************************
    JavaScript *javascript = new JavaScript();
    qml->setContextProperty("_javascript", javascript);
    //*************************************************
    Settings *settings = new Settings();
    qml->setContextProperty("_settings", settings);
    //*********************************************
    //SqlDatabase *sqldatabase = new SqlDatabase();
    //qml->setContextProperty("_sqldatabase", sqldatabase);
    //***************************************************
    //Timer *timer = new Timer();
    //qml->setContextProperty("_timer", timer);
    //***************************************
    //WebImageView *webimageview = new WebImageView();
    //qml->setContextProperty("_webimageview", webimageview);
    //*****************************************************
    XmlReader *xml = new XmlReader();
    qml->setContextProperty("_xml", xml);
    //***********************************

    // Create root object for the UI
    AbstractPane *root = qml->createRootObject<AbstractPane>();

    // Set created root object as the application scene
    Application::instance()->setScene(root);
}

void ApplicationUI::onSystemLanguageChanged()
{
    QCoreApplication::instance()->removeTranslator(m_pTranslator);
    // Initiate, load and install the application translation files.
    QString locale_string = QLocale().name();
    QString file_name = QString("access_permission_%1").arg(locale_string);
    if (m_pTranslator->load(file_name, "app/native/qm")) {
        QCoreApplication::instance()->installTranslator(m_pTranslator);
    }
}
